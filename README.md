...
- **WiFi Download**: Configure a Download URL (e.g., `http://local-server/image.jpg` or `.bmp`), and the ESP32 will fetch the display contents via WiFi upon waking up. 
  - If the server returns a stable `ETag` (preferred) or `Last-Modified`, an unchanged image is not written to flash and does not trigger a panel refresh. Conditional HTTP requests and HTTP 304 are supported. See [UNCHANGED_IMAGE_REFRESH_FIX.md](UNCHANGED_IMAGE_REFRESH_FIX.md).
